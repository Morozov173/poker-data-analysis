...
-- Vice Versa checks for any actions that dont correspond to a game_id
SELECT a.game_id
FROM actions AS a
...
-- Checks for players_games rows without a corresponding game id
SELECT pg.game_id
...
SELECT * FROM players_games WHERE position NOT LIKE 'p_'; 
SELECT * FROM players_games WHERE position NOT REGEXP '^p[1-9]$';
